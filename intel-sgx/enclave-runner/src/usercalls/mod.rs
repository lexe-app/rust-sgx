...
pub(crate) struct ReadOnlyStream(Pin<Box<dyn AsyncRead + Send + Sync + 'static>>);
pub(crate) struct WriteOnlyStream(Pin<Box<dyn AsyncWrite + Send + Sync + 'static>>);

impl ReadOnlyStream {
    pub(crate) fn new(stream: impl AsyncRead + Send + Sync + 'static) -> Self {
        Self(Box::pin(stream))
    }
}

impl WriteOnlyStream {
    pub(crate) fn new(stream: impl AsyncWrite + Send + Sync + 'static) -> Self {
        Self(Box::pin(stream))
    }
}
...
impl AsyncRead for ReadOnlyStream {
    forward!(fn poll_read(mut self: Pin<&mut Self>, cx: &mut Context, buf: &mut ReadBuf) -> Poll<IoResult<()>>);
}

impl AsyncWrite for WriteOnlyStream {
    forward!(fn poll_write(mut self: Pin<&mut Self>, cx: &mut Context, buf: &[u8]) -> Poll<IoResult<usize>>);
    forward!(fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<IoResult<()>>);
    forward!(fn poll_shutdown(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<IoResult<()>>);
}

impl AsyncWrite for ReadOnlyStream {
    fn poll_write(self: Pin<&mut Self>, _cx: &mut Context, _buf: &[u8]) -> Poll<IoResult<usize>> {
...
impl AsyncRead for WriteOnlyStream {
    fn poll_read(
        self: Pin<&mut Self>,
        _cx: &mut Context,
        _buf: &mut ReadBuf,
...
pub(crate) struct EnclaveStdio {
    pub stdin: Option<ReadOnlyStream>,
    pub stdout: Option<WriteOnlyStream>,
    pub stderr: Option<WriteOnlyStream>,
}
...
pub trait AsyncStream: AsyncRead + AsyncWrite + 'static + Send + Sync {
    fn poll_read_alloc(self: Pin<&mut Self>, cx: &mut Context) -> Poll<IoResult<Vec<u8>>> {
        let mut v = vec![0u8; 8192];
        let mut buf = ReadBuf::new(v.as_mut_slice());

        match self.poll_read(cx, &mut buf) {
            Poll::Ready(Ok(())) => {
                let fill_len = buf.filled().len();
                v.truncate(fill_len);
                Poll::Ready(Ok(v))
            }
...
impl AsyncStreamContainer {
    ...
    async fn async_read(&self, buf: &mut [u8]) -> IoResult<usize> {
        poll_fn(|cx| {
            ...
            let mut adapter = match inner.as_mut().poll(cx) {
                Poll::Ready(adapter) => adapter,
                Poll::Pending => return Poll::Pending,
            };
            adapter
                .as_mut()
                .poll_read(cx, &mut buf)
                .map(|res| res.map(|()| buf.filled().len()))
        })
        .await
    }
...
impl PendingEvents {
    ...
    async fn wait_for(&self, event_mask: u64) -> u64 {
        ...
        let ((ev, maybe_permit), _, _) = futures::future::select_all(it).await;
        ...
        if ev != EV_ABORT {
            let permit = maybe_permit.expect("We never call Semaphore::close()");
            permit.forget();
        }
        ev
    }
...
impl EnclaveState {
    ...
    fn new(
        ...
        let stdin = stdio
            .stdin
            .take()
            .unwrap_or_else(|| ReadOnlyStream::new(Stdin));
        let stdout = stdio
            .stdout
            .take()
            .unwrap_or_else(|| WriteOnlyStream::new(tokio::io::stdout()));
        let stderr = stdio
            .stderr
            .take()
            .unwrap_or_else(|| WriteOnlyStream::new(tokio::io::stderr()));
    ...
    fn syscall_loop(
        ...
        let mut rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .thread_name("enclave-runner-syscall-loop")
            .build()
    ...
    pub(crate) fn main_entry(
        ...
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .thread_name("enclave-runner-main")
            .build()
            .expect("failed to create tokio Runtime");
