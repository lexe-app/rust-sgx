...
use crate::usercalls::{EnclaveStdio, ReadOnly, UsercallExtension, WriteOnly};
...
impl<'a> EnclaveBuilder<'a> {
    ...
    pub fn stdin(&mut self, stream: impl AsyncRead + Send + Sync + 'static) -> &mut Self {
        self.stdio.stdin = Some(ReadOnly(Box::pin(stream)));
        self
    }
    ...
    pub fn stdout(&mut self, stream: impl AsyncWrite + Send + Sync + 'static) -> &mut Self {
        self.stdio.stdout = Some(WriteOnly(Box::pin(stream)));
        self
    }
    ...
    pub fn stderr(&mut self, stream: impl AsyncWrite + Send + Sync + 'static) -> &mut Self {
        self.stdio.stderr = Some(WriteOnly(Box::pin(stream)));
        self
    }
